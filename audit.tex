...
The E2E feature of STAR-Vote enables individual voters to confirm that their votes were included in the
...
STAR-Vote uses SOBA \cite{benalohEtal11} as its risk-limiting audit.
...
A big advantage of STAR-Vote is that it records and stores individual cast vote records in a way that 
{\em can\/} be linked to
...
The encrypted vote data on the ballot that STAR-Vote produces serves as a unique ballot identifier,
an ingredient that simplifies SOBA procedures because it eliminates the need to store ballots in a rigid order.
Moreover, because STAR-Vote generates the electronic vote data directly from the touchscreen device,
the same device that prints the voter's selections, the SOBA audit should never find a
discrepancy between the manual interpretation of a paper ballot and the electronic vote data
corresponding to that ballot: If it does, something has gone seriously wrong.

On the other hand, it could easily happen that a ballot enters the ballot box but its barcode is not
